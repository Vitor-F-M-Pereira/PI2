...
if ($_SERVER["REQUEST_METHOD"] === 'POST') {
    ...
    for ($i = 0; $i < $quantidade_aulas; $i++) {
        if (!empty($datas_reposicao[$i]) && !empty($horarios_inicio[$i]) && !empty($horarios_fim[$i])) {
            $sql_aula = "INSERT INTO aulas_reposicao (reposicao_id, data_reposicao, horario_inicio, horario_fim) VALUES (?, ?, ?, ?)";
            $stmt_aula = $conn->prepare($sql_aula);
            $stmt_aula->execute([$id, $datas_reposicao[$i], $horarios_inicio[$i], $horarios_fim[$i]]);
        }
    }
    ...
    if (isset($_FILES['anexo']) && $_FILES['anexo']['error'] === UPLOAD_ERR_OK) {
        ...
        $arquivoNovoCaminho = "uploads/$nomeArquivoNovo";


        if (!empty($reposicao['anexo']) && file_exists("../" . $reposicao['anexo'])) {
            unlink("../" . $reposicao['anexo']);
        }


        $stmt_justificativas = $conn->prepare("UPDATE justificativas SET anexo = ? WHERE id_justificativa = ?");
        $stmt_justificativas->execute([$arquivoNovoCaminho, $id_justificativa]);
    }
    ...
    echo "<script>
            alert('Formulário atualizado e enviado com sucesso!');
    ...
    <div class="page-container">
        <div class="container-editarform">
            ...
            <form action="" method="POST" enctype="multipart/form-data">
                ...
                <?php if (!empty($reposicao['anexo'])): ?>
                    <p>Arquivo atual: <a href="http://localhost/PI2_10_11_quase/uploads/<?= htmlspecialchars($reposicao['anexo']) ?>" target="_blank">Visualizar Anexo</a></p>
                <?php endif; ?>
